<?php
/**
 * LottoExpert.net — SKAI Results Intelligence Page
 * FL1 / Pick 6 (Florida Lotto)
 * Joomla 5.x + PHP 8.1+
 *
 * GAME CONFIG IN THIS FILE:
 * - game_id  = FL1
 * - Pick 6   = 6 main balls, range 01..53
...
$gameMainMax    = 53;
$gameBonusCount = 0;
$gameBallCols   = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth'];
...
if ($dbCol === '' || !preg_match('/^[A-Za-z0-9_#]+$/', $dbCol)) {
    echo '<div style="padding:12px;border:1px solid #f1c40f;background:#fff8e1;border-radius:12px;max-width:980px;margin:12px auto;">'
        . '<strong>Configuration error:</strong> Missing or invalid data source table.</div>';
    echo \JHtml::_('content.prepare', '{loadposition Pick6Wheels}');
    return;
}
...
if ($stateName !== '' && $gName !== '') {
    ...
    $doc->setMetaData(
        'description',
        'Analyze ' . $gName . ' results and number frequency for ' . $stateName
        . '. View the latest draw, most active numbers, quiet stretches, and full historical distribution — '
        . $gameMainMax . ' main numbers tracked over your selected window. Powered by LottoExpert.'
    );
}
 ...
 * --------------------------------------------------------------------------
 */
function fl1FmtDate(?string $date): string
{
    if (!$date) {
        return '';
    }
...
function fl1FmtDateLong(?string $date): string
{
    if (!$date) {
        return '—';
    }
...
function fl1Pad2(string $value): string
{
...
function fl1ResolveLogo(string $stateAbrev, string $gName): array
{
...
function fl1InitRange(int $min, int $max): array
{
    $counts        = [];
    $lastSeenIndex = [];
...
function fl1DrawingsAgoLabel(?int $idx, int $window): array
{
    if ($idx === null) {
...
function fl1BuildNaturalLabels(int $min, int $max): array
{
    $labels = [];
...
function fl1TopKeysByValue(array $counts, int $limit, bool $ascending = false): array
{
    $work = $counts;
...
function fl1FindRepeated(array $latestBalls, array $previousRows, array $ballCols): array
{
    $repeated = [];
...
function fl1CommaList(array $items): string
{
...
function fl1FetchRecentDraws(\Joomla\Database\DatabaseDriver $db, string $dbCol, string $gameId, int $limit): array
{
...
function fl1GetPreviousOccurrence(
...
function fl1EscapeJs(string $value): string
{
    return str_replace(
        ["\\", "'", "\r", "\n", '</'],
        ["\\\\", "\\'", '', '', '<\/'],
 ...
 * --------------------------------------------------------------------------
 */
$latestRows = fl1FetchRecentDraws($db, $dbCol, $gameId, 1);
...
$p1 = $lr ? fl1Pad2(trim((string) ($lr['first']  ?? ''))) : '';
$p2 = $lr ? fl1Pad2(trim((string) ($lr['second'] ?? ''))) : '';
$p3 = $lr ? fl1Pad2(trim((string) ($lr['third']  ?? ''))) : '';
$p4 = $lr ? fl1Pad2(trim((string) ($lr['fourth'] ?? ''))) : '';
$p5 = $lr ? fl1Pad2(trim((string) ($lr['fifth']  ?? ''))) : '';
$p6 = $lr ? fl1Pad2(trim((string) ($lr['sixth']  ?? ''))) : '';

$latestMainBalls = array_filter([$p1, $p2, $p3, $p4, $p5, $p6], static function ($b) {
    return $b !== '';
});
$latestMainBalls = array_values($latestMainBalls);

$logo = ($stateAbrev !== '' && $gName !== '')
    ? fl1ResolveLogo((string) $stateAbrev, (string) $gName)
    : ['exists' => false, 'url' => ''];

/**
 * --------------------------------------------------------------------------
 * Frequency window fetch + PHP counting
 * --------------------------------------------------------------------------
 */
$rowsMain = fl1FetchRecentDraws($db, $dbCol, $gameId, $nodCurrent);

[$mainCounts, $mainLastSeenIndex] = fl1InitRange($gameMainMin, $gameMainMax);
 ...
 * --------------------------------------------------------------------------
 */
$mainChartLabels   = fl1BuildNaturalLabels($gameMainMin, $gameMainMax);
$mainChartValues   = [];
$mainRecencyValues = [];

foreach ($mainChartLabels as $label) {
    $mainChartValues[]   = (int) ($mainCounts[$label] ?? 0);
    $mainRecencyValues[] = ($mainLastSeenIndex[$label] ?? null) === null
        ? ($nodCurrent + 1)
...
/**
 * --------------------------------------------------------------------------
 * Active / quiet insight
 * --------------------------------------------------------------------------
 */
$topActiveKeys = fl1TopKeysByValue($mainCounts, 10, false);
$topQuietKeys  = fl1TopKeysByValue($mainCounts, 10, true);
...
$repeatedNumbers   = fl1FindRepeated($latestMainBalls, array_slice($rowsMain, 1, 10), $gameBallCols);
$mostActiveSummary = array_slice($topActiveKeys, 0, 3);
$quietSummary      = array_slice($quietestKeys, 0, 3);

/**
 * --------------------------------------------------------------------------
 * Window shift: compare 50-draw vs 300-draw leaders
 * --------------------------------------------------------------------------
 */
$window50  = fl1FetchRecentDraws($db, $dbCol, $gameId, 50);
$window300 = fl1FetchRecentDraws($db, $dbCol, $gameId, 300);

[$counts50,]  = fl1InitRange($gameMainMin, $gameMainMax);
[$counts300,] = fl1InitRange($gameMainMin, $gameMainMax);
...
$top50  = fl1TopKeysByValue($counts50, 5, false);
$top300 = fl1TopKeysByValue($counts300, 5, false);
...
$windowChangeNarrative  = 'In the recent 50-draw view, the leading activity centers on ' . fl1CommaList(array_slice($top50, 0, 3)) . '. ';
$windowChangeNarrative .= 'In the broader 300-draw view, ' . fl1CommaList(array_slice($top300, 0, 3)) . ' remains more historically prominent. ';

if (!empty($windowShiftIn)) {
    $windowChangeNarrative .= fl1CommaList(array_slice($windowShiftIn, 0, 2)) . ' gains prominence when the window broadens. ';
}

if (!empty($windowShiftOut)) {
    $windowChangeNarrative .= fl1CommaList(array_slice($windowShiftOut, 0, 2)) . ' looks more concentrated in the shorter recent view.';
}
...
if ($drawDate !== '') {
    foreach ($latestMainBalls as $ball) {
        $prevDate = fl1GetPreviousOccurrence($db, $dbCol, $gameId, $drawDate, $ball, $gameBallCols);
        $drawsAgo = fl1DrawingsSinceDate($db, $dbCol, $gameId, $prevDate, $drawDate);
 ...
 * --------------------------------------------------------------------------
 */
$heroInsight  = 'Latest verified draw and recent number behavior for ' . htmlspecialchars((string) $gName, ENT_QUOTES, 'UTF-8') . ' at a glance. ';
$heroInsight .= 'Review the most active numbers, quiet stretches, and full historical frequency before moving into deeper SKAI analysis.';
...
.skai-chart-frame--tall{height:1100px}
...
$jsonLdPage = [
    ...
    'name'        => htmlspecialchars_decode(
        'Results Intelligence — ' . (string) $stateName . ' • ' . (string) $gName . ' | LottoExpert.net'
    ),
    'description' => 'Lottery number frequency analysis and results intelligence for '
        . (string) $gName . ' — ' . (string) $stateName . '. '
        . 'Covers ' . (int) $gameMainMax . ' main numbers across historical draw windows.',
...
$jsonLdDataset = [
    '@context'    => 'https://schema.org',
    '@type'       => 'Dataset',
    'name'        => htmlspecialchars_decode((string) $gName . ' Number Frequency Data'),
    'description' => 'Historical draw frequency for ' . (string) $gName . ' main numbers 01 to '
        . (string) str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT)
        . ' across up to ' . (int) $nodCurrent . ' recent drawings.',
    'creator'     => ['@type' => 'Organization', 'name' => 'LottoExpert.net'],
];

$jsonLdEncoded1 = json_encode($jsonLdPage, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
$jsonLdEncoded2 = json_encode($jsonLdDataset, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
?>
<script type="application/ld+json"><?php echo $jsonLdEncoded1 !== false ? $jsonLdEncoded1 : '{}'; ?></script>
<script type="application/ld+json"><?php echo $jsonLdEncoded2 !== false ? $jsonLdEncoded2 : '{}'; ?></script>
    ...
    <div class="skai-hero-inner">
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <p class="skai-hero-summary">
            <?php echo htmlspecialchars($heroInsight, ENT_QUOTES, 'UTF-8'); ?>
          </p>

          <div class="skai-ball-row" aria-label="Latest drawn numbers (<?php echo (int) $gameMainCount; ?> main balls)">
            <?php foreach ([$p1, $p2, $p3, $p4, $p5, $p6] as $ball) : ?>
              <?php if ($ball !== '') : ?>
                <span class="skai-ball skai-ball--main"><?php echo htmlspecialchars(fl1Pad2($ball), ENT_QUOTES, 'UTF-8'); ?></span>
              <?php endif; ?>
            <?php endforeach; ?>
          </div>

          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?game_id=FL1">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=FL1">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=FL1">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=FL1">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/all-lottery-heatmaps?gameId=FL1">Heatmap Analysis</a>
          </div>
        </div>
          ...
          <div class="skai-meta-stack">
            <div class="skai-meta-row">
              <div class="skai-meta-box">
                ...
                <span class="value"><?php echo htmlspecialchars(fl1FmtDateLong($drawDate), ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
              <div class="skai-meta-box">
                <span class="label">Next draw date</span>
                <span class="value"><?php echo htmlspecialchars(fl1FmtDateLong($nextDrawDate), ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
            </div>
            ...
            <?php
            $jackpotOk = ($nextJackpot !== '' && $nextJackpot !== 'n/a'
                          && is_numeric($nextJackpot) && (float) $nextJackpot > 0);
            ?>
            <?php if ($jackpotOk) : ?>
              <div class="skai-meta-box">
  ...
  <section class="skai-strip" aria-label="Key takeaways">
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        <div class="skai-stat-value"><?php echo htmlspecialchars(fl1CommaList($mostActiveSummary), ENT_QUOTES, 'UTF-8'); ?></div>
        <div class="skai-stat-note">Highest appearance counts in the current <?php echo (int) $nodCurrent; ?>-draw main-number window.</div>
      </div>
    </article>
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        <div class="skai-stat-value"><?php echo htmlspecialchars(fl1CommaList($quietSummary), ENT_QUOTES, 'UTF-8'); ?></div>
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        <div class="skai-stat-value"><?php echo htmlspecialchars(!empty($repeatedNumbers) ? fl1CommaList($repeatedNumbers) : 'None', ENT_QUOTES, 'UTF-8'); ?></div>
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        ...
        <div class="skai-stat-note">Main-number draw window currently loaded for this page view. Adjust in the tables section.</div>
      </div>
    </article>
  </section>
    ...
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Start with a clear high-level view. This layer is designed for fast orientation: which numbers are most active,
          which are quiet, and how the current draw relates to recent history. Numbers range from
          <?php echo str_pad((string) $gameMainMin, 2, '0', STR_PAD_LEFT); ?> to
          <?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?>
          across <?php echo (int) $gameMainCount; ?> main draws per game.
        </p>
      </div>
    </div>
  ...
  <section id="recency-deep-dive" class="skai-section" aria-labelledby="recency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          This section makes the current draw easier to interpret. It shows how recently each of the
          <?php echo (int) $gameMainCount; ?> latest main numbers was previously seen and how the perspective
          shifts when you compare a short recent window with a broader historical one.
        </p>
      </div>
    </div>
      ...
      <div class="skai-two-col">
        <div class="skai-card">
          ...
          <div class="skai-card-body">
                  ...
                  <div class="skai-history-item">
                    ...
                    <div class="skai-history-date">
                      <?php if (!empty($row['prevDate'])) : ?>
                        Previously seen on <?php echo htmlspecialchars(fl1FmtDateLong((string) $row['prevDate']), ENT_QUOTES, 'UTF-8'); ?>
                      <?php else : ?>
            ...
            <?php else : ?>
              <div class="skai-empty">Draw history data is not available for the current window.</div>
            <?php endif; ?>
          </div>
        </div>
          ...
          <div class="skai-card-body">
            <div class="skai-window-shift">
              ...
              <div class="skai-shift-panel">
                ...
                <p class="skai-shift-text"><?php echo htmlspecialchars(fl1CommaList(array_slice($top50, 0, 5)), ENT_QUOTES, 'UTF-8'); ?></p>
              </div>

              <div class="skai-shift-panel">
                <p class="skai-shift-label">Broader 300-draw leaders</p>
                <p class="skai-shift-text"><?php echo htmlspecialchars(fl1CommaList(array_slice($top300, 0, 5)), ENT_QUOTES, 'UTF-8'); ?></p>
              </div>
            </div>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The left panel shows the complete main-number distribution across all
          <?php echo (int) $gameMainCount; ?>-ball values
          <?php echo str_pad((string) $gameMainMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?>.
          The right panel shows recency distribution — how many draws since each number last appeared.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values <?php echo str_pad((string) $gameMainMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?> across the last <?php echo (int) $nodCurrent; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--ember">
            ...
            <span class="skai-card-sub">Distance since last appearance for each main number (01&ndash;<?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?>)</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
              <div class="skai-chart-frame">
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick
        reading; this section is optimized for thorough review across the full <?php echo str_pad((string) $gameMainMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?> range.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the table for exact counts, recency, and personal tracking across all
          <?php echo (int) ($gameMainMax - $gameMainMin + 1); ?> main-ball values.
          Quick filters help narrow the view without losing full access to the complete dataset.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-card">
        <div class="skai-card-head skai-card-head--horizon">
          Main numbers table
          <span class="skai-card-sub">Exact counts and recency for values <?php echo str_pad((string) $gameMainMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?></span>
        </div>
          ...
          <table id="skai-main-table" class="skai-table" aria-label="Main numbers frequency table">
            ...
            <tbody>
              <?php for ($i = $gameMainMin; $i <= $gameMainMax; $i++) : ?>
                ...
                [$lastDrawSort, $lastDrawLabel] = fl1DrawingsAgoLabel($mainLastSeenIndex[$number] ?? null, (int) $nodCurrent);
    ...
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          ...
          for <?php echo htmlspecialchars((string) $gName, ENT_QUOTES, 'UTF-8'); ?>.
        </p>
      </div>
    </div>
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            <p class="skai-tool-copy">
              Best next step for a broader multi-signal view. Use this after reviewing frequency and recency to move into
              the main SKAI intelligence workflow for <?php echo htmlspecialchars((string) $gName, ENT_QUOTES, 'UTF-8'); ?>.
            </p>
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?game_id=FL1">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=FL1">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            <p class="skai-tool-copy">
              Useful for comparing appearance spacing and interruption behavior after reviewing current frequency data.
            </p>
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=FL1">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=FL1">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/all-lottery-heatmaps?gameId=FL1">Heatmap Analysis</a>
        <a class="skai-utility-link" href="/lottery-archives-pick6?gId=<?php echo rawurlencode($gId); ?>&amp;stateName=<?php echo rawurlencode((string) $stateName); ?>&amp;gName=<?php echo rawurlencode((string) $gName); ?>&amp;sTn=<?php echo rawurlencode(strtolower((string) $stateAbrev)); ?>">Lottery Archives</a>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing
        draw history, but they should be treated as descriptive signals rather than guarantees. This page covers
        <?php echo htmlspecialchars((string) $gName, ENT_QUOTES, 'UTF-8'); ?> —
        a Pick <?php echo (int) $gameMainCount; ?> game drawing numbers
        <?php echo str_pad((string) $gameMainMin, 2, '0', STR_PAD_LEFT); ?> to <?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?>.
        The purpose of this page is to make recent behavior easier to understand, compare, and carry into deeper SKAI analysis.
      </div>
    </div>
  </section>
...
(function () {
  ...
  var chartData = {
    ...
    mainRecency:     <?php echo json_encode(array_values($mainRecencyValues)); ?>
  };
  ...
  function renderCharts() {
    ...
    if (topActiveCanvas && chartData.topActiveLabels && chartData.topActiveLabels.length) {
      new window.Chart(topActiveCanvas.getContext('2d'), {
        type: 'bar',
        data: {
    ...
    if (quietCanvas && chartData.quietLabels && chartData.quietLabels.length) {
      new window.Chart(quietCanvas.getContext('2d'), {
        type: 'bar',
        data: {
    ...
    if (fullMainCanvas && chartData.mainLabels && chartData.mainLabels.length) {
      new window.Chart(fullMainCanvas.getContext('2d'), {
        type: 'bar',
        data: {
    ...
    if (recencyCanvas && chartData.mainLabels && chartData.mainLabels.length) {
      new window.Chart(recencyCanvas.getContext('2d'), {
        type: 'bar',
        data: {
          labels:   chartData.mainLabels,
          datasets: [{
            data:            chartData.mainRecency,
            borderWidth:     0,
            borderRadius:    6,
            backgroundColor: '#F5A623'
          }]
...
<?php echo \JHtml::_('content.prepare', '{loadposition Pick6Wheels}'); ?>
